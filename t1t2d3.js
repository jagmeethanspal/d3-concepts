var margin = 30,
    ...
    height = 600,
    gMargin = 100;
var offsetChartWidth = 800,
    offsetChartHeight = 300;
var lines, tScale;
const lValues = [];
const rValues = [];
...
function displayOffset(data) {
    ...
    yAxisG.call(yAxis).attr("transform", "translate(" + gMargin + ",0)");
...
function delta(data) {
    ...
    data.forEach((element) => {
        ...
        if (ot1[0] == t1[0]) {
            ...
            t1D = (t1[0] - ot1[0]) * 1000000000;
            t1D += t1[1] - ot1[1];
        }
        if (ot2[0] == t2[0]) {
            t2D = t2[1] - ot2[1];
        } else {
            t2D = (t2[0] - ot2[0]) * 1000000000;
            t2D += t2[1] - ot2[1];
        }
        d.push({
            T2: element.t2,
...
function display(data) {
    ...
    lAxisG = svg.append("g").attr("id", "lAxis").attr("class", "axis");

    lAxisG.call(lAxis).attr("transform", "translate(" + margin * 3 + ",0)");

    rAxis = d3.axisRight(tScale).ticks(10, "+f");
    rAxisG = svg.append("g").attr("id", "rAxis").attr("class", "axis");
